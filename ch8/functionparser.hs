...
eval emv val@(Number _) = return val
eval env val@(Bool _) = return val
eval env (Atom idx) = getVar env idx
eval env (List [Atom "quote" , val]) = return  val
eval env (List [Atom "if" , predc , conseq , alt]) = 
  do result <- eval env predc
     case result of
...
unpackEquals arg1 arg2 (AnyUnpacker unpacker) = 
  ...
  `catchError` const (return False) --don't know why
...
setVar envRef var value = do
  env <- liftIO $ readIORef envRef
  maybe (throwError $ UnboundVar "Seting an unbound variable" var) (liftIO.(flip writeIORef value)) (lookup var env)
  return value
...
main = do
  ...
  case length args of
    0 -> runRepl
    1 -> runOne $ args !! 0
    otherwise -> putStrLn "Programm takes only 0 or 1 argument"
